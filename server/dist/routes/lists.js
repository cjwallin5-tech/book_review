...
router.get("/:id", (req, res) => {
  ...
  `).all(req.params.id);
    const likeCount = db.prepare("SELECT COUNT(*) as count FROM list_likes WHERE list_id = ?").get(req.params.id);
    let isLiked = false;
    if (userId) {
        const like = db.prepare("SELECT id FROM list_likes WHERE list_id = ? AND user_id = ?").get(req.params.id, userId);
        isLiked = !!like;
    }
...
router.post("/", authMiddleware, (req, res) => {
    ...
    const result = db.prepare("INSERT INTO lists (user_id, title, description, is_private) VALUES (?, ?, ?, ?)").run(req.userId, title, description || "", is_private ? 1 : 0);
...
router.put("/:id", authMiddleware, (req, res) => {
    ...
    db.prepare("UPDATE lists SET title = COALESCE(?, title), description = COALESCE(?, description), is_private = COALESCE(?, is_private), updated_at = datetime('now') WHERE id = ?").run(title || null, description !== undefined ? description : null, is_private !== undefined ? (is_private ? 1 : 0) : null, req.params.id);
...
router.post("/:id/items", authMiddleware, (req, res) => {
    ...
    const maxPos = db.prepare("SELECT COALESCE(MAX(position), -1) as m FROM list_items WHERE list_id = ?").get(req.params.id);
    try {
...
router.delete("/:id/items/:bookId", authMiddleware, (req, res) => {
    const list = db.prepare("SELECT id FROM lists WHERE id = ? AND user_id = ?").get(req.params.id, req.userId);
    if (!list) {
        res.status(404).json({ error: "List not found or not yours" });
        return;
...
router.post("/:id/comments", authMiddleware, (req, res) => {
    ...
    const result = db.prepare("INSERT INTO list_comments (list_id, user_id, content) VALUES (?, ?, ?)").run(req.params.id, req.userId, content.trim());
