...
// --- Threads ---
router.get("/categories/:id/threads", (req, res) => {
...
router.post("/categories/:id/threads", authMiddleware, (req, res) => {
    ...
    const result = db.prepare("INSERT INTO discussion_threads (category_id, user_id, title, body, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)").run(req.params.id, req.userId, title.trim(), body?.trim() || "", now, now);
...
router.post("/threads/:id/posts", authMiddleware, (req, res) => {
    ...
    const result = db.prepare("INSERT INTO discussion_posts (thread_id, user_id, content) VALUES (?, ?, ?)").run(req.params.id, req.userId, content.trim());
    db.prepare("UPDATE discussion_threads SET post_count = post_count + 1, updated_at = ? WHERE id = ?").run(new Date().toISOString(), req.params.id);
...
router.delete("/posts/:id", authMiddleware, (req, res) => {
    const post = db.prepare("SELECT id, thread_id FROM discussion_posts WHERE id = ? AND user_id = ?").get(req.params.id, req.userId);
    if (!post) {
        res.status(404).json({ error: "Post not found or not yours" });
        return;
    }
    db.prepare("DELETE FROM discussion_posts WHERE id = ?").run(req.params.id);
    db.prepare("UPDATE discussion_threads SET post_count = post_count - 1 WHERE id = ?").run(post.thread_id);
