...
async function enrichBook(bookId, title, author) {
    try {
        ...
        const current = db.prepare("SELECT genre, country FROM books WHERE id = ?").get(bookId);
        if (!current)
            return;
...
export async function scheduleEnrichment() {
    ...
    if (countriesSet)
        console.log(`[enrich] Set ${countriesSet} countries from curated map.`);
    ...
    const noGenre = db
        ...
        .all();
    if (!noGenre.length) {
        console.log("[enrich] All genres already set.");
        return;
    }
...
router.get("/:id", (req, res) => {
  ...
  `).get(req.params.id);
    ...
    const seriesBooks = seriesName
      ...
      `).all(userId ?? null, seriesName, req.params.id)
        : [];
    // Current user's rating for this book (rating-only or full review)
    let userRating = null;
    if (userId) {
        const row = db.prepare("SELECT rating FROM reviews WHERE book_id = ? AND user_id = ? ORDER BY created_at DESC LIMIT 1").get(req.params.id, userId);
...
router.post("/:id/reviews", authMiddleware, (req, res) => {
    ...
    if (trimmedContent === "") {
        ...
        const existing = db.prepare("SELECT id FROM reviews WHERE book_id = ? AND user_id = ? AND content = ''").get(req.params.id, user.id);
        if (existing) {
            db.prepare("UPDATE reviews SET rating = ? WHERE id = ?").run(rating, existing.id);
            reviewId = existing.id;
        }
        else {
            const result = db.prepare("INSERT INTO reviews (book_id, user_id, user_name, rating, content) VALUES (?, ?, ?, ?, '')").run(req.params.id, user.id, user.username, rating);
            reviewId = result.lastInsertRowid;
        }
    }
    else {
        // Full review with text
        const result = db.prepare("INSERT INTO reviews (book_id, user_id, user_name, rating, content) VALUES (?, ?, ?, ?, ?)").run(req.params.id, user.id, user.username, rating, trimmedContent);
        reviewId = result.lastInsertRowid;
    }
...
router.get("/:id/tbr", authMiddleware, (req, res) => {
...
router.get("/:id/reading-status", authMiddleware, (req, res) => {
    const entry = db.prepare("SELECT id FROM reading_log WHERE user_id = ? AND book_id = ? AND end_date IS NULL").get(req.userId, req.params.id);
    res.json({ reading: !!entry });
});
router.post("/:id/reading", authMiddleware, (req, res) => {
    const bookId = Number(req.params.id);
    const book = db.prepare("SELECT id FROM books WHERE id = ?").get(bookId);
    if (!book) {
        res.status(404).json({ error: "Book not found" });
    ...
    const existing = db.prepare("SELECT id FROM reading_log WHERE user_id = ? AND book_id = ? AND end_date IS NULL").get(req.userId, bookId);
    if (existing) {
        ...
        db.prepare("INSERT INTO reading_log (user_id, book_id, start_date, format) VALUES (?, ?, ?, ?)").run(req.userId, bookId, today, "paperback");
        res.json({ reading: true });
    }
});
// --- Similar books (collaborative filtering with genre fallback) ---
router.get("/:id/similar", (req, res) => {
    const book = db.prepare("SELECT id, genre FROM books WHERE id = ?").get(req.params.id);
    if (!book) {
        res.status(404).json({ error: "Book not found" });
        return;
    }
  ...
  `).all(book.genre, req.params.id);
    ...
    const seen = new Set(collaborative.map((b) => b.id));
    const merged = [...collaborative, ...genreBased.filter((b) => !seen.has(b.id))].slice(0, 8);
    res.json(merged);
});
// --- Reading progress ---
router.get("/reading/progress", authMiddleware, (req, res) => {
    const progress = db.prepare("SELECT book_id, current_page, total_pages FROM reading_progress WHERE user_id = ?").all(req.userId);
    res.json(progress);
});
router.get("/series/progress", authMiddleware, (req, res) => {
...
router.get("/:id/progress", authMiddleware, (req, res) => {
    const row = db.prepare("SELECT current_page, total_pages FROM reading_progress WHERE user_id = ? AND book_id = ?").get(req.userId, req.params.id);
