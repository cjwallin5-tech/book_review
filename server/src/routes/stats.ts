...
router.get("/:id", (req: Request, res: Response) => {
  ...
  const readThisYear = db.prepare(
    "SELECT COUNT(*) as count FROM reading_log WHERE user_id = ? AND strftime('%Y', COALESCE(end_date, start_date)) = ?"
  ...
  const avgRating = db.prepare(
    "SELECT COALESCE(AVG(rating), 0) as avg FROM reviews WHERE user_id = ?"
  ...
  const diaryEntries = db.prepare(
    "SELECT COALESCE(end_date, start_date) as date FROM reading_log WHERE user_id = ? ORDER BY date DESC"
  ...
  res.json({
    ...
    favorite_author: favoriteAuthor ? (favoriteAuthor as { author: string; count: number }).author : null,
    current_streak: currentStreak,
    genre_breakdown: genreBreakdown as { genre: string; count: number }[],
  });
});
