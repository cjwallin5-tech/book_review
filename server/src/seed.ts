...
const existing = db.prepare("SELECT COUNT(*) as count FROM books").get() as { count: number };
...
async function fetchBookDetails(title: string) {
  try {
...
async function main() {
  ...
  const books: { title: string; author: string; cover_url: string; description: string; genre: string }[] = [];
